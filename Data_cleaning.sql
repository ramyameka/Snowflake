/*
An E-commerce company noticed that the number of inactive customers is rising. So they decided to re engage with those customers through a new marketing campaign.
As an analyst, your task is to find the list of inactive customers who didn't make any transactions in the last 90 days, the final output will feed the marketing campaign, the available data in our systems came with many data quality issues, duplicated customer records, missing values, merged columns, non-standardized phone numbers and wrong data types.
In addition, extra fields needs to be derived from dates like days since last transaction which will help find the appropriate list of inactive customers. 
*/

-- Step 1 Use the Snowflake UI to Create DATABASE PROJECT2 

-- Step 2 Create Table CUSTOMERS under PROJECT2.PUBLIC ---
...
-- Step 3 Load Data Manually or use CSV to load the data
...
-- Use the Result Panel from UI

-- USE UI to setup the context (Warehouse, database and schema) OR run SQL SCRIPT 

-- Step 4 Use CONCAT() to Visualize Spaces in the "NAME" Column 

SELECT NAME, concat('>', NAME, '<') FROM CUSTOMERS;

-- Step 5 Use TRIM() to Remove leading and trailing characters from column "NAME" 

SELECT NAME, TRIM(NAME, ' 0') as N, concat('>', N, '<') FROM CUSTOMERS;

-- Step 6 Use SPLIT_PART() to split a Text column into 2 parts using ',' as a delimiter 

SELECT NAME, SPLIT_PART(TRIM(NAME, ' 0'), ',', 1) as first_name,  SPLIT_PART(TRIM(NAME, ' 0'), ', ', 2) as last_name FROM CUSTOMERS;

-- Step 7 Use LTRIM TO REMOVE + and 0  from "PHONE" Column

SELECT PHONE, LTRIM(PHONE, '0+') as STANDARD_PHONE from CUSTOMERS;

-- Step 8 Use TO_DATE() convert Text column into Date by defining the input format


SELECT to_date(DOB, 'MMMM DD, YYYY'), to_date(LASTTRANSACTION, 'AUTO') FROM CUSTOMERS;

-- Step 9 Use DATEDIFF() to count the Number of days between LASTTRANSACTION Column and CURRENT_DATE() in "DAYS"  

SELECT to_date(LASTTRANSACTION, 'AUTO'), datediff(days, to_date(LASTTRANSACTION, 'AUTO'), CURRENT_DATE()) as DAYSSINCE_LASTTRANSACTION
FROM CUSTOMERS;


-- Step 10 Using the DaysSinceLastTransaction column Query the DataSet to find the list of Inactive Customers (DaysSinceLastTransaction>90) 
...
-- Step 11 Use DATEDIFF() to count Age of each Customer in "YEARS"

SELECT to_date(DOB, 'MMMM DD, YYYY') as DOB, datediff('year', to_date(DOB, 'MMMM DD, YYYY'), CURRENT_DATE()) as AGE
FROM CUSTOMERS;

--  Combine all in one Select Statement  ---
...
-- Step 12 Handle Missing Data with Deletion 

SELECT * FROM CUSTOMERS where NOT (email is null or email = '');

-- Step 13 Handle Missing Data with Imputation 

SELECT COMPANY, IFF(COMPANY is null or COMPANY='', 'NA', COMPANY) as CLEAN_COMPANY from CUSTOMERS;


-- Step 14 Find Duplicated Customers based on email ---

SELECT EMAIL, COUNT(ID) FROM CUSTOMERS GROUP BY EMAIL HAVING COUNT(ID) >1;

-- Step 15 Eliminate Duplicated Customers and keep the record with the latest Transaction Date ---

SELECT ID, NAME, EMAIL, LASTTRANSACTION ,
rank() over (partition by email order by to_date(LASTTRANSACTION, 'AUTO') desc) as RANKS
FROM CUSTOMERS QUALIFY RANKS =1;


-- Step 16 Create a View to scale and save your work, Bring together all bits and pieces from previous tasks    
...
-- Step 17 Query CUSTOMERS_VW to find the list of inactive Customers then export results in CSV 

SELECT * FROM CUSTOMER_VW WHERE DAYSSINCELASTTRANS >90;
